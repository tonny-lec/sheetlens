...
`process-project-backlog` が通常完了した後は、独立した `$promote-ready-project-issues` handoff が
`scripts/promote_ready_issues.py` を呼び出し、条件を満たす `proposed` 課題だけを `ready` へ昇格します。
新たに ready になった課題は、次回の backlog 処理で選択します。
...
- 同一の不変スナップショットだけを読む `check`、`next`、diff/status の確認は並列化できるが、
  status/owner 更新、`render`、検証、commit の依存列と read-only 確認を同時実行しない。
...
2026-07-10 に管理基盤、初期 17 課題、全検証が完了し、bootstrap の委譲例外は終了しました。
現在は通常運用です。管理ファイルの新規作成と、課題の状態、owner、依存関係、受け入れ条件、
完了証拠、backlog の更新は親ワーカーだけが行います。実装ワーカーは担当コードと関連テスト
だけを変更します。
